...
export class Item extends MyObject3D {
  ...
  private _isRotCall:boolean = true
    ...
    this._tri = new Mesh(
      ...
      new ShaderMaterial({
        ...
        uniforms:{
          ...
          dotSize:{value:1},
          shape:{value:1},
          simpleColRate:{value:0},
          simpleCol:{value:new Color()},
  ...
  // ---------------------------------
  public update():void {
    this._isRotCall = false
    this._update()
    this._isRotCall = true
  }
  ...
  protected _update():void {
    ...
    const s = Param.instance.seek

    const sukima = 0.75
    this._con.position.y = 0
    const num = Param.instance.rotNum
    let it = (Conf.instance.SCROLL_HEIGHT * sh - sh) / num
    ...
    const size = sw / Param.instance.line
    this._con.scale.set(size, size, 1)

    this.itemSize.x = size
    this.itemSize.y = size

    this._setUni(this._tri, 'shape', Param.instance.shape)
    this._setUni(this._tri, 'r', new Vector2(size, size))
    this._setUni(this._tri, 'simpleColRate', Param.instance.simpleCol)
    this._setUni(this._tri, 'simpleCol', Param.instance.bgColor)
